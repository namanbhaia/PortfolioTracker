            ...
            >
                <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200/60 dark:border-slate-800 transition-colors">
                    ...
                    {isSubmitted ? (
                        ...
                        >
                            <div className="w-16 h-16 bg-green-50 dark:bg-green-950/30 rounded-full flex items-center justify-center mx-auto mb-6 transition-colors">
                                <CheckCircle2 className="w-8 h-8 text-green-500" />
                            </div>
                            <h2 className="text-2xl font-semibold mb-2 text-slate-900 dark:text-white transition-colors">Check your email</h2>
                            <p className="text-slate-500 dark:text-slate-400 mb-8 transition-colors">
                                We've sent a password reset link to <span className="font-semibold text-slate-700 dark:text-slate-300 transition-colors">{email}</span>.
                            </p>
                            <button
                                onClick={() => router.push('/login')}
                                className="w-full py-4 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-semibold rounded-2xl transition-all"
                            >
                                Return to Login
                            </button>
                        </motion.div>
                        ...
                        <>
                            <h2 className="text-2xl font-semibold mb-4 text-slate-900 dark:text-white transition-colors">Reset Password</h2>
                            ...
                            <form onSubmit={handleReset} className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500"
                                        />
                                    </div>
                                </div>
                                ...
                                <button
                                    type="submit"
                                    disabled={isLoading}
                                    className="w-full py-4 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-semibold rounded-2xl shadow-lg shadow-blue-200 dark:shadow-none transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                                >
                                    {isLoading ? (
