...
export default function DashboardPage() {
    ...
    const selectedNames = searchParams.get('clients')?.split(',').filter(Boolean) || [];
    ...
    useEffect(() => {
        const fetchHoldings = async () => {
            ...
            try {
                ...
                const { data: rawHoldings, error: queryError } = await query;
                ...
                const processedRows = Object.values(aggregatedMap).map((item: any) => {
                    ...
                    return { ...item, avg_purchase_price, pl, pl_percent };
                }).sort((a, b) => a.ticker.localeCompare(b.ticker));

                setConsolidatedRows(processedRows);
    ...
    if (userError) {
        return <div className="p-6 text-center text-red-500">Error loading user data: {userError}</div>;
    }
