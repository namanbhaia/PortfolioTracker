...
export default function Sidebar({ user, profile }: { user: any, profile?: any }) {
    ...
    return (
        <aside
            className={`relative h-screen bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 flex flex-col border-r border-slate-200 dark:border-slate-800 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-20' : 'w-64'
                }`}
        >
            ...
            <div className="p-6 mb-2 flex items-center gap-3 overflow-hidden">
                <div
                    onClick={() => {
                ...
                >
                    ...
                    {!isCollapsed && (
                        <span className="font-bold text-lg tracking-tight whitespace-nowrap animate-in fade-in slide-in-from-left-2 duration-300 text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                            MLBPortfolioTracker
                        </span>
                    )}
                </div>
                    ...
                    return (
                        <Link
                            ...
                            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${isActive
                                ...
                                : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 dark:hover:text-slate-100 hover:text-slate-900'
                                }`}
                        >
                            <item.icon
                                size={20}
            ...
            <div className="px-3 space-y-1 mb-4">
                {secondaryItems.map((item) => (
                    <Link
                        key={item.name}
                        href={item.href}
                        className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 transition-colors group"
                    >
                        <item.icon size={18} className="shrink-0 text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300" />
                        {!isCollapsed && <span className="truncate">{item.name}</span>}
                    </Link>
                ))}
            </div>
                        ...
                        return (
                            <Link
                                ...
                                className={`flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all group ${isActive
                                    ? 'bg-rose-600/20 text-rose-600 dark:text-rose-400 border border-rose-500/30'
                            ...
                            >
                                <item.icon size={18} className={`shrink-0 ${isActive ? 'text-rose-600 dark:text-rose-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`} />
                                {!isCollapsed && <span className="truncate">{item.name}</span>}
                            </Link>
                        );
                    })}
            ...
            {/* User Profile Footer */}
            <div className={`p-4 border-t border-slate-200 dark:border-slate-800 ${isCollapsed ? 'px-2' : 'px-4'}`}>
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <button className={`flex items-center gap-3 rounded-xl transition-all outline-none ${isCollapsed
                            ? 'p-2 justify-center hover:bg-slate-100 dark:hover:bg-slate-800 w-full'
                            : 'p-3 w-full bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-800'
                            }`}>
